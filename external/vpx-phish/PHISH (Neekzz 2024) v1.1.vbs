...
Sub DT1_dropped
	PlaySoundAt SoundFXDOF("PH - TweePrise",111,DOFPulse,DOFDropTargets), DT1
'	PlaySoundAt "PH - TweePrise", DT1
	if l1.state=1 then
		scorebonus
	  else
		addscore 500
...
Sub DT2_dropped
	PlaySoundAt SoundFXDOF("PH - TweePrise",111,DOFPulse,DOFDropTargets), DT2
'	PlaySoundAt "PH - TweePrise", DT2
	if l2.state=1 then
		scorebonus
	  else
		addscore 500
...
Sub DT3_dropped
	PlaySoundAt SoundFXDOF("PH - TweePrise",111,DOFPulse,DOFDropTargets), DT3
'	PlaySoundAt "PH - TweePrise", DT3
	if l3.state=1 then
		scorebonus
	  else
		addscore 500
...
Sub DT4_dropped
	PlaySoundAt SoundFXDOF("PH - TweePrise",111,DOFPulse,DOFDropTargets), DT4
'	PlaySoundAt "PH - TweePrise", DT4
	if l4.state=1 then
		scorebonus
	  else
		addscore 500
...
Sub DT5_dropped
	PlaySoundAt SoundFXDOF("PH - TweePrise",111,DOFPulse,DOFDropTargets), DT5
'	PlaySoundAt "PH - TweePrise", DT5
	if l5.state=1 then
		scorebonus
	  else
		addscore 500
...
Sub DT6_dropped
	PlaySoundAt SoundFXDOF("PH - TweePrise",116,DOFPulse,DOFDropTargets), DT6
'	PlaySoundAt "PH - TweePrise", DT6
	if l6.state=1 then
		scorebonus
	  else
		addscore 500
...
Sub DT7_dropped
	PlaySoundAt SoundFXDOF("PH - TweePrise",116,DOFPulse,DOFDropTargets), DT7
'	PlaySoundAt "PH - TweePrise", DT7
	if l7.state=1 then
		scorebonus
	  else
		addscore 500
...
Sub DT8_dropped
	PlaySoundAt SoundFXDOF("PH - TweePrise",116,DOFPulse,DOFDropTargets), DT8
'	PlaySoundAt "PH - TweePrise", DT8
	if l8.state=1 then
		scorebonus
	  else
		addscore 500
...
Sub DT9_dropped
	PlaySoundAt SoundFXDOF("PH - TweePrise",116,DOFPulse,DOFDropTargets), DT9
'	PlaySoundAt "PH - TweePrise", DT9
	if l9.state=1 then
		scorebonus
	  else
		addscore 500
...
Sub DT10_dropped
	PlaySoundAt SoundFXDOF("PH - TweePrise",116,DOFPulse,DOFDropTargets), DT10
'	PlaySoundAt "PH - TweePrise", DT10
	if l10.state=1 then
		scorebonus
	  else
		addscore 500
...
'*************************** PuP Background Music for this table ********************************
' by NailBuster v0.1
' The idea is to allow your own custom background music/control during gameplay.
' You just have to add your own mp3s into a subfolder/playlist of PuPVideos\myMusic.
' You can have as many playlists inside of myMusic to share amongst different vpx tables
'
'  PuPVideos\myMusic\Top40 
'  PuPVideos\myMusic\BritneySpears 
'  PuPVideos\myMusic\NickleBack 
'  PuPVideos\myMusic\TopRAP 
'
'  do not add this table script to tables that are already using PuPlayer or PuPPacks.
'
' 
'  step 1>  if Not already there manually create a subfolder in PupVideos called "MyMusic" without quotes.
'   
'  step 2>  create a subfolder inside myMusic and add as many mp3 as you like, this folder name is the Playlist Name.  
'
'  step 3>  copy/paste this PuP Background Music script to end of vpx table script
'
'  step 4>  find in table script Table1_KeyDown and ADD first line of sub like below
'
'                    Sub Table1_KeyDown(ByVal keycode)
'                        PuPOnKeyDown(keycode)
'
'  step 5>  find in table script Table1_KeyUP and ADD first line of sub like below
'
'                    Sub Table1_KeyUP(ByVal keycode)
'                        PuPOnKeyUp(keycode)
'
'  step 6>  set that subfolder (aka Playlist) below for cPuPPlayList
'
'  Thats It!   Now using Magna buttons will go next/prior in playlist.
'  if you HOLD left flipper down,  then using left/right magna will lower/increase volume.
'
'  note:  below is a pupRandomize value.  1=enable 0=alpha sort for how to initially load the mps 
'
'*************************************************************************************************
Dim usePUP: Dim PuPlayer: Dim PUPStatus: PUPStatus=false : Dim pLeftFlipperDown: pLeftFlipperDown = 0 ' dont edit this line!!!


dim cPuPPlayList : cPuPPlayList = "PHISH-ABRA"        ' name of subfolder/playlist in your myMusic PupPack folder (located in PuPVideos).
dim cPuPPack     : cPuPPack = "myMusic"          ' name of the PuP-Pack / PuPVideos folder for this table
dim pupCurVolume : pupCurVolume = 90             ' default pupvolume. 1-100 
dim pAudio       : pAudio=12		  	 ' leave 12 for virtual pup display
dim pupVolumeStep: pupVolumeStep = 5             ' volume change steps 
dim pupRandomize : pupRandomize = 0              ' 0=alpha sort 1= randomize
...
Sub PuPOnKeyDown(keycode)
If PUPStatus=false then Exit Sub
if keycode=LeftFlipperKey Then pLeftFlipperDown = 1
end Sub

Sub PuPOnKeyUp(keycode)
If PUPStatus=false then Exit Sub
if keycode=LeftMagnaSave AND pLeftFlipperDown = 0  Then PuPlayListPrior pAudio
if keycode=RightMagnaSave AND pLeftFlipperDown = 0  Then PuPlayListNext pAudio

if keycode=RightMagnaSave AND pLeftFlipperDown = 1 Then 
    pupCurVolume = pupCurVolume + pupVolumeStep
    if pupCurVolume>100 then pupCurVolume = 100
    PuPlayer.setVolumeCurrent pAudio, pupCurVolume
    PuPlayer.setVolume pAudio, pupCurVolume
end If

if keycode=LeftMagnaSave AND pLeftFlipperDown = 1 Then 
    pupCurVolume = pupCurVolume - pupVolumeStep
    if pupCurVolume<1 then pupCurVolume = 1
    PuPlayer.setVolumeCurrent pAudio, pupCurVolume
    PuPlayer.setVolume pAudio, pupCurVolume
end If
if keycode=LeftFlipperKey Then pLeftFlipperDown = 0
end Sub
...
Sub PuPStart(cPuPPack)
    ...
    If usePUP=true then
        ...
        Else
            ...
            PuPlayer.Init pAudio, cPuPPack
            PuPlayer.SetScreenex pAudio,0,0,0,0,2
	    PuPlayer.hide pAudio
            PuPlayer.playlistadd pAudio, cPuPPlayList, pupRandomize, 0
            PUPDisplayAsJukebox pAudio
            PuPlayer.setvolume pAudio, pupCurVolume 
            PuPlayer.PlayListPlay pAudio,cPuPPlayList
            PUPStatus=true
        End If
    End If
End Sub
...
'**********************   END OF PUP BACKGROUND MUSIC SCRIPT  **************************************