...
Sub RollingUpdate()
    ...
    For b = lob to UBound(BOT)
        ...
        If bvel > 1 Then				   
			...
			Else
				StopSound BallRollStr(b)
				PlaySound MetalRollStr(b), -1, bvol, bpan, 0, bpitch, 1, 0, bfade
			End if
...
Flip1RStartAngle = RightFlipper1.StartAngle
Flip1LStartAngle = LeftFlipper1.StartAngle
...
Sub Realtime_Timer
	...
	Dim rf1CA : rf1CA = RightFlipper1.CurrentAngle
	Dim lf1CA : lf1CA = LeftFlipper1.CurrentAngle

	primFlipperR.roty=-FlipRStartAngle+rfCA
	primFlipperL.roty=-FlipLStartAngle+lfCA
	primFlipperROff.roty=-FlipRStartAngle+rfCA
	primFlipperLOff.roty=-FlipLStartAngle+lfCA

	PrimRightFlipper1.RotY=-Flip1RStartAngle+rf1CA
	PrimLeftFlipper1.RotY=-Flip1LStartAngle+lf1CA
...
'WriteToLog "     ", "FLASHFLASHER!!!! " & nr
	...
	objlight(nr).IntensityScale = 0.5 * FlasherLightIntensity * ol3
	objbase(nr).BlendDisableLighting = FlasherOffBrightness + 10 * ol3
	objlit(nr).BlendDisableLighting = 10 * ol2
	UpdateMaterial FlasherMatStr(nr),0,0,0,0,0,0,ol,RGB(255,255,255),0,0,False,True,0,0,0,0
	ObjLevel(nr) = ol * 0.9 - 0.01
...
Class LampFader
	...
	Public UseFunction
	Private cFilter
	Public UseCallback(140), cCallback(140), cCallbackRef(140)
