...
' VR-Hybrid Mod by RobbyKingPin, TastyWasps, Retro27, hauntfreaks, HayJay, Nestorgian, fluffhead35, DaRdog81, LoadedWeapon, Themer & DGrimmReaper - 2023/2026
'
' This mod contains the following improvements:
'
' RobbyKingPin:
' - nFozzy/Rothbauerw physics,Fleep sound with a lot of custom sounds from VPW's Rollercoaster Tycoon and VPW Dynamic Ball Shadows (Updated v.2.4)
' - Rothbauerw Standup target codes (Updated v.2.4)
...
' - More light tweaks to more modern standards based on VPW's settings (Updated v.2.3)
...
' - Updated PWM on flashers GI and Inserts, added 3D inserts and converted 1 pair of inserts and Allspark to work with Flupper Domes (Updated v.2.3)
' - Added new apron primitive and flipperbats from VPW's AD/DC and created a new custom apron on top. Added sidewood in the playfield cutouts. Converted png images to webp (Updated v.2.3)
' - Slingshots are solenoid driven, based on VPW's Spider-Man Classic (v.2.2)
' - Updated trough code based on VPW's AC/DC (v.2.2)
' - Converted Flupper Round Domes to be linked to the lamp matrix but still controlled by the Flupper Domes codes (Updated v.2.3)
' - Updated Flipper Dampeners (v.2.2)
' - Changed a few settings on the Flupper Inserts to make them look good (Updated v.2.3)
' - Added an upscaled texture for the custom apron provided by cheese3075 (v.2.2)
' - VPW Tweak Menu added  with newly added LUT and a great amount of customized items to choose to replace the old VPW Options UI (Updated v.2.3.1)
' - Big uprez in the playfield using AI (v.2.3.1)
' - Created new backpanel lights (v.2.4)
' - Baked textures for the sidewalls, custom blades, backpanel, plastics, decals, ramps, all the toys, Allspark and playfield shadows (v.2.4)
...
'
' Retro27:
' - Updated VR Cabinet using assets from DaRdog and improved textures (Updated v.2.4)
...
'
' LoadedWeapon (v.2.2):
' - Created a custom sideblades primitive to fit on Legends Cabinets and provided a secondary ini file for the POV to match (Updated v.2.3)
'
' Themer (v.2.2):
...
' Special thanks to everybody involved during development and testing: Smaug, PinStratsDan, Primetime5K, Hay Jay, gtxjoe, bietekwiet, Wylte, BountyBob, Nestorgian, fluffhead35, redbone, guus_8005, geradg, Studlygoorite, 
' Sinizin, mason, Burger, kilonova, supergibson, chokeee
'
'Shoutout to:
...
Dim bsTrough, bsSaucer, bsKickerMegaTron, plungerIM, cbRight, CapBall, CarBall, x
...
Sub li1_Animate()
	If li1.state > 0.05 Then
		Pincab_Start_Button.disablelighting = 0.100
	Else
...
Sub li2_Animate()
	If li2.state > 0.05 Then
		Pincab_Tournament_Button.disablelighting = 0.100
	Else
		Pincab_Tournament_Button.disablelighting = 0
	End if
End Sub

Dim FlasherSevenIntensity, FlasherEightIntensity, FlasherNineIntensity, FlasherTenIntensity
Dim LastFlasherSevenIntensity, LastFlasherEightIntensity, LastFlasherNineIntensity, LastFlasherTenIntensity
LastFlasherSevenIntensity = -1
LastFlasherEightIntensity = -1
LastFlasherNineIntensity = -1
LastFlasherTenIntensity = -1

Sub li57_Animate()
	FlasherTenIntensity = li57.GetInPlayIntensity
	If FlasherTenIntensity <> LastFlasherTenIntensity Then
		ModFlashFlasher 10,FlasherTenIntensity
		LastFlasherTenIntensity = FlasherTenIntensity
	End If
End Sub

Sub li60_Animate()
	FlasherSevenIntensity = li60.GetInPlayIntensity
	If FlasherSevenIntensity <> LastFlasherSevenIntensity Then
		ModFlashFlasher 7,FlasherSevenIntensity
		LastFlasherSevenIntensity = FlasherSevenIntensity
	End If
End Sub

Sub li61_Animate()
	FlasherEightIntensity = li61.GetInPlayIntensity
	If FlasherEightIntensity <> LastFlasherEightIntensity Then
		ModFlashFlasher 8,FlasherEightIntensity
		LastFlasherEightIntensity = FlasherEightIntensity
	End If
End Sub

Sub li62_Animate()
	FlasherNineIntensity = li62.GetInPlayIntensity
	If FlasherNineIntensity <> LastFlasherNineIntensity Then
		ModFlashFlasher 9,FlasherNineIntensity
		LastFlasherNineIntensity = FlasherNineIntensity
	End If
End Sub
...
Sub Table1_Init
	...
	Set Capball = CapKicker1.CreateSizedballWithMass(Ballsize/2,Ballmass)
    Set Carball = carkicker.CreateSizedballWithMass(Ballsize/2,Ballmass)
    Carball.Visible = 0
...
Sub Table1_KeyDown(ByVal Keycode)
	If keycode = LeftFlipperKey Then
		...
		PinCab_Left_Flipper_Button.X = PinCab_Left_Flipper_Button.X + 4
	End If
	If keycode = RightFlipperKey Then
		FlipperActivate RightFlipper, RFPress
		PinCab_Right_Flipper_Button.X = PinCab_Right_Flipper_Button.X - 4
	End If
	...
	If keycode = StartGameKey Then 
		...
		Pincab_Start_Button_Decal.Y = Pincab_Start_Button_Decal.Y -3
	End If
	If keycode = keyFront Then 
		Controller.Switch(15) = True
		soundStartButton() 
		Pincab_Tournament_Button.Y =Pincab_Tournament_Button.Y -2 'Tournament mode mapped to key 2
	End If
...
Dim Purple, PurpleFull
PurpleFull = rgb(128,0,255)
Purple = rgb(64,0,64)

Dim Red, RedFull
RedFull = rgb(184,15,10)
Red = rgb(85,6,4)
...
Sub GI_PWM(nr, aLvl)
	...
	Dim ii, xx
	For each ii in GIBulb : ii.state = aLvl : Next
	For each ii in GILeft : ii.state = aLvl : Next
	For each ii in GIRight : ii.state = aLvl : Next
	For each ii in GIRightRed : ii.state = aLvl : Next
	For each ii in GITopLeft : ii.state = aLvl : Next
	For each ii in GITopRight : ii.state = aLvl : Next
	For each ii in GIPlasticLeft : ii.state = aLvl : Next
	For each ii in GIPlasticRight : ii.state = aLvl : Next
	For each ii in GIPlasticRightRed : ii.state = aLvl : Next
	If Not GIisOn And aLvl > 0.5 Then
		...
		If GIColorMod = 0 Then
			...
			BackpanelGlass.image = "BackpanelGlassGIOn"
			BackpanelSteel.image = "BackpanelSteelGIOn"
			Flasherbase6.image = "AllSparkGIOn"
			Ironhide.image = "IronhideGIOn"
			LeftJet.image = "LeftJetGIOn"
			LeftRampPlastic.image = "RampGIOn"
			LeftRampPlasticClear.image = "LeftRampPlasticClearGIOn"
			LeftRampPlasticClearTop.image = "LeftRampPlasticClearTopGIOn"
			Megatron.image = "MegatronGIOn"
			OptimusPrime.image = "OptimusGIOn"
			RampPlastics.image = "RampPlasticsGIOn"
			RightJet.image = "RightJetGIOn"
			RightRampPlastic.image = "RampGIOn"
			RightRampPlasticEntry.image = "RightRampPlasticEntryGIOn"
			RightRampFlap.image = "RampGIOn"
			RightRampMetal.image =  "RightRampMetalGIOn"
			Starscream.image = "StarscreamGIOn"
			TransformersPlastics.image = "TransformersPlasticsGIOn"
			If SideBladeMod = 0 Then
				Pincab_Blades.image = "PinCab_BladesGIOn"
			End If
			If SideBladeMod = 1 Then
				Pincab_Blades.image = "PinCab_Blades_Custom1GIOn"
			End If
			If SideBladeMod = 2 Then
				Pincab_Blades.image = "PinCab_Blades_Custom2GIOn"
			End If
			If SideBladeMod = 3 Then
				Pincab_Blades.image = "PinCab_Blades_Custom3GIOn"
			End If
			If SideBladeMod = 4 Then
				Pincab_Blades.image = "PinCab_Blades_Custom4GIOn"
			End If
			If SideBladeMod = 5 Then
				Pincab_Blades.image = "PinCab_Blades_Custom5GIOn"
			End If
		Else
			Backpanel.image = "BackwallGIOnLED"
			BackpanelFilament.image = "BackpanelFilamentGIOnLED"
			BackpanelGlass.image = "BackpanelGlassGIOnLED"
			BackpanelSteel.image = "BackpanelSteelGIOnLED"
			Flasherbase6.image = "AllSparkGIOnLED"
			Ironhide.image = "IronhideGIOnLED"
			LeftJet.image = "LeftJetGIOnLED"
			LeftRampPlastic.image = "RampGIOnLED"
			LeftRampPlasticClear.image = "LeftRampPlasticClearGIOnLED"
			LeftRampPlasticClearTop.image = "LeftRampPlasticClearTopGIOnLED"
			Megatron.image = "MegatronGIOnLED"
			OptimusPrime.image = "OptimusGIOnLED"
			RampPlastics.image = "RampPlasticsGIOnLED"
			RightJet.image = "RightJetGIOnLED"
			RightRampPlastic.image = "RampGIOnLED"
			RightRampPlasticEntry.image = "RightRampPlasticEntryGIOnLED"
			RightRampFlap.image = "RampGIOnLED"
			RightRampMetal.image =  "RightRampMetalGIOnLED"
			Starscream.image = "StarscreamGIOnLED"
			TransformersPlastics.image = "TransformersPlasticsGIOnLED"
			If SideBladeMod = 0 Then
				Pincab_Blades.image = "PinCab_BladesGIOnLED"
			End If
			If SideBladeMod = 1 Then
				Pincab_Blades.image = "PinCab_Blades_Custom1GIOnLED"
			End If
			If SideBladeMod = 2 Then
				Pincab_Blades.image = "PinCab_Blades_Custom2GIOnLED"
			End If
			If SideBladeMod = 3 Then
				Pincab_Blades.image = "PinCab_Blades_Custom3GIOnLED"
			End If
			If SideBladeMod = 4 Then
				Pincab_Blades.image = "PinCab_Blades_Custom4GIOnLED"
			End If
			If SideBladeMod = 5 Then
				Pincab_Blades.image = "PinCab_Blades_Custom5GIOnLED"
			End If
		End If
		If SideBladeMod = 0 Then
			Pincab_Blades.image = "PinCab_BladesGIOn"
	...
	Elseif GIisOn And aLvl < 0.4 Then
		...
		BackpanelGlass.image = "BackpanelGlassGIOff"
		BackpanelSteel.image = "BackpanelSteelGIOff"
		Flasherbase6.image = "AllSparkGIOff"
		Ironhide.image = "IronhideGIOff"
		LeftJet.image = "LeftJetGIOff"
		LeftRampPlastic.image = "RampGIOff"
		LeftRampPlasticClear.image = "LeftRampPlasticClearGIOff"
		LeftRampPlasticClearTop.image = "LeftRampPlasticClearTopGIOff"
		Megatron.image = "MegatronGIOff"
		OptimusPrime.image = "OptimusGIOff"
		RampPlastics.image = "RampPlasticsGIOff"
		RightJet.image = "RightJetGIOff"
		RightRampPlastic.image = "RampGIOff"
		RightRampPlasticEntry.image = "RightRampPlasticEntryGIOff"
		RightRampFlap.image = "RampGIOff"
		RightRampMetal.image =  "RightRampMetalGIOff"
		Starscream.image = "StarscreamGIOff"
		TransformersPlastics.image = "TransformersPlasticsGIOff"
		If SideBladeMod = 0 Then
			Pincab_Blades.image = "PinCab_BladesGIOff"
		End If
		If SideBladeMod = 1 Then
			Pincab_Blades.image = "PinCab_Blades_Custom1GIOff"
		End If
		If SideBladeMod = 2 Then
			Pincab_Blades.image = "PinCab_Blades_Custom2GIOff"
		End If
		If SideBladeMod = 3 Then
			Pincab_Blades.image = "PinCab_Blades_Custom3GIOff"
		End If
		If SideBladeMod = 4 Then
			Pincab_Blades.image = "PinCab_Blades_Custom4GIOff"
		End If
		If SideBladeMod = 5 Then
			Pincab_Blades.image = "PinCab_Blades_Custom5GIOff"
		End If
		Sound_GI_Relay 0, Bumper1
	End If
	PinCab_Blades.blenddisablelighting = 0.2 * aLvl
	Backpanel.blenddisablelighting = 0.75 * aLvl
	metals_prim.blenddisablelighting = .6 * aLvl
	PFShadowGIOn.opacity = aLvl * 60
	PFShadowGIOff.opacity = 60 - (aLvl * 60)
	gilvl = alvl
End Sub
...
Dim DSSources(28), numberofsources', DSGISide(30) 'Adapted for TZ with GI left / GI right
...
Class Dampener
	...
	Public Sub Dampen(aBall)
		...
		End If
		If aBall.id > UBound(cor.ballvelx) Then cor.Update	'new balls can hit before the next frame tick tracks them
...
Sub RollingUpdate()
	Dim b
	Dim BOT
	cor.Update	'track per-ball velocity; sizes the CoRTracker arrays that Apron_Hit/STCheckHit index by ball id (this call was missing -> Subscript out of range at line 3586)
	BOT = GetBalls
...
Sub Apron_Hit (idx)
	If ActiveBall.id > UBound(cor.ballvelx) Then cor.Update	'new balls can hit before the next frame tick tracks them
...
Sub SolModFlasherPWM118(level)
	ModFlashFlasher 3,level
End Sub

Sub SolModFlasherPWM121(level)
	ModFlashFlasher 5,level
End Sub

Sub SolModFlasherPWM123(level)
	ModFlashFlasher 4,level
End Sub

Sub SolModFlasherPWM132(level)
...
Dim OutlaneDifficulty, ApronMod, DecalMod, InstMod, FlipperMod, GIColorMod, JetMod, SpotLightMod, GlassMod, DMDDecalMod, DMDDetail,TopperMod, LogoPosterMod, Railsmod
...
Sub Table1_OptionEvent(ByVal eventId)
	...
	'Dynamic BallShadow
	DynamicBallShadowsOn = Table1.Option("Dynamic Ballshadow", 0, 1, 1, 1, 0, Array("Off", "On"))
	'Reflections
	...
	'Custom Flippers
	FlipperMod = Table1.Option("Flipperbats", 0, 2, 1, 0, 0, Array("Original", "White Bats Purple Rubbers", "Transformers Red & Purple Rubbers"))
	SetupFlippers
	'Sideblades
	SideBladeMod = Table1.Option("Side Blades", 0, 5, 1, 0, 0, Array("Original", "Red", "Retrorefurbs 1", "Tiltgraphics", "Wizardmod", "Retrorefurbs 2"))
	SetupBlades
	'GI
	GIColorMod = Table1.Option("GI", 0, 1, 1, 0, 0, Array("White bulbs and GI", "Purple and red GI"))
	SetupGIMod
	'Custom Jets
...
Sub SetRenderProbes(Opt)		
	On Error Resume Next
		Select Case Opt
			Case 0:
				Primitive48.RefractionProbe = ""
				Primitive67.RefractionProbe = ""
			Case 1:
				Primitive48.RefractionProbe = "LeftRamp"
				Primitive67.RefractionProbe = "RightRamp"
		End Select
	On Error Goto 0
End Sub
...
Sub SetupOutlanes
	If OutlaneDifficulty = 0 Then
		zCol_Rubber_Post020.Collidable = False
		zCol_Rubber_Post008.Collidable = False
		zCol_Rubber_Post021.Collidable = True
		RubberBand10.Collidable = False
		RubberBand8.Collidable = False
		RubberBand11.Collidable = True
		...
		OutlaneScrewLeft1.Visible = False
		OutlaneScrewLeft2.Visible = False
		OutlaneScrewLeft3.Visible = True
		zCol_Rubber_Post022.Collidable = False
		zCol_Rubber_Post010.Collidable = False
		zCol_Rubber_Post023.Collidable = True
		RubberBand12.Collidable = False
		RubberBand9.Collidable = False
		RubberBand13.Collidable = True
		...
		OutlaneScrewRight1.Visible = False
		OutlaneScrewRight2.Visible = False
		OutlaneScrewRight3.Visible = True
	End If
	If OutlaneDifficulty = 1 Then
		zCol_Rubber_Post020.Collidable = False
		zCol_Rubber_Post008.Collidable = True
		zCol_Rubber_Post021.Collidable = False
		RubberBand10.Collidable = False
		RubberBand8.Collidable = True
		RubberBand11.Collidable = False
		OutlaneRubberLeft1.Visible = False
		...
		OutlaneScrewLeft1.Visible = False
		OutlaneScrewLeft2.Visible = True
		OutlaneScrewLeft3.Visible = False
		zCol_Rubber_Post022.Collidable = False
		zCol_Rubber_Post010.Collidable = True
		zCol_Rubber_Post023.Collidable = False
		RubberBand12.Collidable = False
		RubberBand9.Collidable = True
		RubberBand13.Collidable = False
		OutlaneRubberRight1.Visible = False
		...
		OutlaneScrewRight1.Visible = False
		OutlaneScrewRight2.Visible = True
		OutlaneScrewRight3.Visible = False
	End If
	If OutlaneDifficulty = 2 Then
		zCol_Rubber_Post020.Collidable = True
		zCol_Rubber_Post008.Collidable = False
		zCol_Rubber_Post021.Collidable = False
		RubberBand10.Collidable = True
		RubberBand8.Collidable = False
		RubberBand11.Collidable = False
		...
		OutlanePegLeft2.Visible = False
		OutlanePegLeft3.Visible = False
		OutlaneScrewLeft1.Visible = True
		OutlaneScrewLeft2.Visible = False
		OutlaneScrewLeft3.Visible = False
		zCol_Rubber_Post022.Collidable = True
		zCol_Rubber_Post010.Collidable = False
		zCol_Rubber_Post023.Collidable = False
		RubberBand12.Collidable = True
		RubberBand9.Collidable = False
		RubberBand13.Collidable = False
		...
		OutlanePegRight2.Visible = False
		OutlanePegRight3.Visible = False
		OutlaneScrewRight1.Visible = True
		OutlaneScrewRight2.Visible = False
		OutlaneScrewRight3.Visible = False
	End If
End Sub
...
Sub SetupFlippers
	If FlipperMod = 0 Then
		LFLogo.image = "BatsWhiteRed"
		RFLogo.image = "BatsWhiteRed"
	End If
	If FlipperMod = 1 Then
		LFLogo.image = "BatsWhitePurple"
		RFLogo.image = "BatsWhitePurple"
	End If
	If FlipperMod = 2 Then
		LFLogo.image = "BatsWhitePurpleTF"
		RFLogo.image = "BatsWhiteRedTF"
	End If
End Sub
...
Sub SetupGIMod
	Dim coll, obj
	For Each coll in Array(GILeft, GITopLeft)
		For Each obj In coll
			If GIColorMod = 1 Then
				obj.Color	  = Purple
				obj.ColorFull = PurpleFull
				obj.intensity = 30
			Else
				obj.Color	  = White
		...
		Next
	Next
		For Each coll in Array(GIPlasticLeft)
		For Each obj In coll
			If GIColorMod = 1 Then
				obj.Color	  = Purple
				obj.ColorFull = PurpleFull
		...
		Next
	Next
	For Each coll in Array(GIRight, GITopRight)
		For Each obj In coll
			If GIColorMod = 1 Then
				obj.Color	  = Red
		...
		Next
	Next
	For Each coll in Array(GIPlasticRight)
		For Each obj In coll
			If GIColorMod = 1 Then
				obj.Color	  = Red
				obj.ColorFull = RedFull
		...
		Next
	Next
	For Each coll in Array(GIRightRed)
		For Each obj In coll
			If GIColorMod = 1 Then
				obj.intensity = 30
			Else
		...
		Next
	Next
	For Each coll in Array(GIPlasticRightRed)
		For Each obj In coll
			If GIColorMod = 1 Then
				obj.intensity = 15
			Else
...
Sub SetupJets
	If JetMod = 1 Then
		LeftJet.Visible = True
		RightJet.Visible = True
	Else
...
Sub SetupRailsMod
	If Table1.ShowDT = False And Table1.ShowFSS = False And RenderingMode <> 2 Then
		If RailsMod = 1 Then
...
Sub SetupDMDDecal
	If RenderingMode = 2 or Table1.ShowFSS = True or VRTest = 1 Then
		If DMDDecalmod = 0 Then
			Pincab_DMD_Decal.visible = False
			PinCab_DMD_Decal_Custom.visible = False
			PinCab_DMD_Decal_Custom_HD.visible = False
			PinCab_Grills_Left.visible = True
			PinCab_Grills_Right.visible = True
		End If
		...
		If DMDDecalmod = 4 Then
			...
			If DMDDetail = 1 Then
				Pincab_DMD_Decal_Custom.visible = False
				Pincab_DMD_Decal_Custom_HD.visible = True
				PinCab_Grills_Left.visible = False
				PinCab_Grills_Right.visible = False
			Else
				Pincab_DMD_Decal.visible = False
...
Sub SetupTopper
	If RenderingMode = 2 or Table1.ShowFSS = True or VRTest = 1 Then
		If TopperMod = 1 Then
			PinCab_Tournament_Topper.Visible = True
		Else
			PinCab_Tournament_Topper.Visible = False
		End If
	End If
End Sub
